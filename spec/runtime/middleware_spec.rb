...
RSpec.describe RailsContractSync::Runtime::Middleware do
  ...
  def app
    ...
    RailsContractSync::Runtime::Middleware.new(
      inner,
      store: @store,
  ...
  it "does not record when disabled" do
    Dir.mktmpdir do |dir|
      disabled_store = RailsContractSync::Runtime::ObservationStore.new(File.join(dir, "obs.jsonl"))
      inner = lambda do |_env|
        [200, { "Content-Type" => "application/json" }, ['{"ok":true}']]
      end
      middleware = RailsContractSync::Runtime::Middleware.new(
        inner,
  ...
  it "buffers the body so a single-enumeration response is not double-consumed" do
    store = RailsContractSync::Runtime::ObservationStore.new(File.join(@dir, "once.jsonl"))
    json = { "id" => 1 }.to_json
    ...
    mw = RailsContractSync::Runtime::Middleware.new(inner, store: store, route_resolver: ->(_e) { "/things" }, enabled: true)
  ...
  it "does not record non-JSON responses" do
    store = RailsContractSync::Runtime::ObservationStore.new(File.join(@dir, "html.jsonl"))
    inner = ->(_env) { [200, { "Content-Type" => "text/html" }, ["<h1>hi</h1>"]] }
    mw = RailsContractSync::Runtime::Middleware.new(inner, store: store, route_resolver: ->(_e) { "/x" }, enabled: true)
    mw.call(Rack::MockRequest.env_for("/x"))
    expect(store.all).to eq([])
  end

  it "does not record when the route resolver returns nil" do
    store = RailsContractSync::Runtime::ObservationStore.new(File.join(@dir, "noroute.jsonl"))
    inner = ->(_env) { [200, { "Content-Type" => "application/json" }, ["{}"]] }
    mw = RailsContractSync::Runtime::Middleware.new(inner, store: store, route_resolver: ->(_e) { nil }, enabled: true)
    mw.call(Rack::MockRequest.env_for("/x"))
    expect(store.all).to eq([])
  end
end
